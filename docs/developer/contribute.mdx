...
- [.docs/architecture.md](https://github.com/KitsuneKode/kunai/blob/main/.docs/architecture.md) — playback flow and data ownership (in-repo deep doc, not published on this site)
...
All three should pass before you open a PR. Run `bun run fmt` if formatting fails.
...
Isolated installer smoke (does not touch host `~/.local/bin`):
...
- Do not register candidate providers in `bootstrap-providers.ts` until they pass the quality gate.
