...
`generate` must run before `typecheck` and `build` when invoking scripts directly. Turborepo runs it once via `apps/docs/turbo.json` task dependencies.
</Step>
</Steps>
...
`apps/docs/scripts/sync-repo-content.ts` bakes the monorepo files the site renders but must not read at request time — `.release/*.json`, the troubleshooting FAQ parsed out of `docs/`, and the OG mascot PNG — into `lib/generated-release-notes.json`, `lib/generated-troubleshooting-faq.json`, and `lib/generated-mascot.json`. Reading those from disk inside a route made Turbopack trace the whole workspace into every function's NFT list; the runtime now touches no filesystem. `scripts/check-codegen-freshness.ts` fails when any generated file drifts from its source.

Run `bun run --cwd apps/docs generate` after changing commands, providers, help text, `docs/feature-status.yaml`, `docs/users/troubleshooting.mdx`, `docs/troubleshooting-symptoms.yaml`, or `.release/`.
...
CI runs `generate` once, then `typecheck:app`, `test`, and `build:app` (skipping redundant MDX/codegen passes). The freshness gate compares fingerprints and registry payloads, not volatile `syncedAt` / `cliSourceRevision` stamps.
