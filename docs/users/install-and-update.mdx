...
Requires Bun on your `PATH` (the published entry is `#!/usr/bin/env bun`):
...
Global npm installs write `install.json` automatically via bundled `postinstall` so ownership-aware `kunai upgrade` / `kunai uninstall` route correctly. Diagnose PATH shadows with `kunai doctor`.
...
| macOS x64/arm64 (Apple Silicon) | Beta |
| Windows x64 | Beta |
| Windows ARM64 | Experimental |
| FreeBSD / BSD | Not published — use npm/bun/source (contributor-oriented) |
