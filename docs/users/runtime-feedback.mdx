...
- Press `/memory` while playback is active to reveal the memory strip.
- By default, the strip is hidden and temporary.
- If the memory panel setting is enabled, `/memory` pins it until you run `/memory` again.
- Live `m` during playback opens the title control menu, not the memory strip.
