...
tests and temporary sessions. The relay is metadata-only. Stream URLs stay
direct; `videoFallback` is persisted in config but has no production reader, so
turning it on does not proxy video.
