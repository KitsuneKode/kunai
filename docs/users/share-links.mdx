...
After registration on Linux, `kunai://` links from browsers or chat apps route through `--handoff-url`, which may ask for confirmation. On macOS/Windows, keep using `kunai --open`.
