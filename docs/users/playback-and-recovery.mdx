...
AniSkip for split-cour anime: when the only MAL mapping is TMDB season-1, Kunai **skips AniSkip for season 2+** rather than applying the wrong show's windows. The default anime provider, Miruro, is keyed on AniList, where each cour is its own entry with its own MAL id, so that gap does not arise there; AniDB stamps MAL at resolve when it can.
