...
3. **Bootstrap a known id** — `-i` / `--id` skips the initial title search for that id. A bare or `tmdb:<id>` id needs `-t movie` or `-t tv` (tv = series) and is not supported together with anime mode (`-a`). Namespaced ids reuse the share-link grammar — `anilist:<id>` and `mal:<id>` imply anime mode, `youtube:<id>` implies YouTube mode; `imdb:` is rejected.
