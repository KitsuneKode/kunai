...
8. **Experimental Google Cast** — `--cast` opens the playback-device picker; `--cast <device>` targets a receiver directly; `--cast-list` discovers receivers and exits.
9. **Discover / calendar / random** — `--discover`, `--calendar`, `--random`: open those trays first without auto-play.
10. **Share link handoff** — `--open <kunai://url>`: trusted launch without protocol confirmation; `--install-protocol-handler` registers the **Linux-only** OS handler.
...
- **Google Cast:** `--cast` keeps the existing full video+audio path through Google's Default Media Receiver. **Audio-only Cast is experimental:** `--cast-audio [device]` uses Kunai's Custom Web Receiver, requires `KUNAI_CAST_RECEIVER_APP_ID` plus `ffmpeg`, plays video locally with mpv audio disabled, and sends receiver clock updates back to Kunai. A bare `--cast` only offers the experimental audio-only choice when that application ID is configured. Full A/V Cast publishes selected SRT, ASS, and WebVTT subtitles as receiver text tracks. `--cast-list` prints discovered receivers and exits. `--cast <name|ip>` selects a receiver directly for HTTP(S) HLS, DASH, MP4, WebM, or MP3 playback. Header-protected streams use a tokenized, session-scoped LAN gateway that closes with playback. During playback, Space pauses/resumes, Left/Right seek 10 seconds, and clicking the progress bar seeks directly.
