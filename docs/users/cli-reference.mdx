...
- **Player selection:** `--player auto|mpv|vlc` chooses the backend for one run.
  Desktop `auto` and `mpv` use supervised mpv; desktop VLC is not supported.
  Android `auto` opens the chooser, while `mpv` and `vlc` target those installed
  Android apps.
...
Android handoff reports launch acceptance only. Kunai cannot observe external
progress, completion, EOF, track changes, or player controls, so it does not
infer history progress, autoplay, or queue acknowledgement from the intent.
