...
- **Google Cast (experimental):** `--cast` opens a Kunai picker with local playback, discovered receivers, refresh, and manual address entry. `--cast-list` prints discovered receivers and exits. `--cast <name|ip>` selects a receiver directly for HTTP(S) HLS, DASH, MP4, WebM, or MP3 playback. Header-protected HTTP(S) streams use a tokenized, session-scoped gateway on this device; it closes when playback ends or is stopped.
