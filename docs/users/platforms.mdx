...
- `curl` — used by the default anime provider (Miruro) and by AniDB, both behind Cloudflare
...
Miruro (the default anime provider) and AniDB both sit behind Cloudflare. Kunai
tries its own HTTP client first and falls back to `curl` when Cloudflare refuses
it — which, at Miruro's API, is every time, and at AniDB's, often. A provider
