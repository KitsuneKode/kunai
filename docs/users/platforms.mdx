...
Miruro (the default anime provider) and AniDB both sit behind Cloudflare, and
Kunai reaches them through `curl` rather than its own HTTP client, which
Cloudflare refuses at Miruro's API and often at AniDB's. Cloudflare scores the
TLS handshake, not just the
`User-Agent`, so a plain `curl` is sometimes refused too even though the site is
up. Either way anime search can come back with **no results** rather than an
