...
Anonymous usage analytics are **yours to turn on or off at any time**, in
...
people — but nothing is enabled until you confirm it on that screen. Skipping
...
- **Your IP address is never read.** Not collected and discarded — the ingest
  has no code path that reads a client address at all, so there is nothing to
  log, rate-limit on, or hand over
...
- A day ends at **midnight IST (18:30 UTC)** from 15 September 2026. Days before
  that end at midnight UTC, and 14 September 2026 is a shorter changeover day
  whose count reads low. The "updated" time on the analytics page is shown in
  your own clock; the day totals are not re-cut to your local midnight, because
  only daily totals are kept and the individual pings behind them are not.
