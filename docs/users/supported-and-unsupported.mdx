...
- **Bun / npm global** — secondary; npm needs **Node** on `PATH` (Node launcher). `bun install -g` still needs Bun
...
| Miruro (`miruro`) | Anime | Active default |
| KickAssAnime (`kickassanime`) | Anime | Active, second in the anime order |
| AnimeGG (`animegg`) | Anime | Active, third in the anime order |
| AniDB (`anidb`) | Anime | Active, fourth in the anime order |
| AllManga-compatible (`allanime`) | Anime | Active, last in the anime order |
...
| yt-dlp | YouTube and downloads | YouTube play/resolve and the download queue need it; other online playback does not |
