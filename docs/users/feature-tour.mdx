...
Release binaries via `install.sh` (Linux/macOS) and `install.ps1` (Windows) are **beta supported**. They embed the Bun runtime — you do not need Bun installed to run them. Unsigned binaries may trigger SmartScreen or Gatekeeper on first run; see [Install and update](/docs/users/install-and-update).

| Platform | Install | Playback | Downloads | Poster previews |
| --- | --- | --- | --- | --- |
| Linux | `install.sh` or bun/npm global | mpv | yt-dlp + ffprobe | Kitty / chafa |
| macOS | `install.sh` or bun/npm global | mpv | yt-dlp + ffprobe | chafa |
| Windows | `install.ps1` or bun/npm global | mpv | yt-dlp | chafa (limited) |
