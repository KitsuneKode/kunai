...
Live registry — same table as [Providers](/docs/users/providers#active-providers). Provider names link to per-provider notes.
...
- Movie and series resolve can use the registered Videasy, VidLink, and Rivestream adapters
- Anime resolve starts with Miruro, then KickAssAnime, then AnimeGG, then AniDB, then AllManga (`allanime`); every registered anime provider is tried automatically
...
- Share links (`/share`, `/watch`, `kunai --open`)
...
- AniList and TMDB sync for watch progress (experimental; fail-closed until the promotion gate passes)
...
- Native app store packages (use `install.sh` / `install.ps1` or package managers instead)
...
| Browse results | `Tab` | Cycle catalog mode (series / anime / YouTube) |
| Browse results | `Ctrl+F` | Narrow loaded results |
| Browse results | `Enter` | Open or play selected result |
| Browse results | `Shift+Enter` / `i` | Open title details |
| Browse results | `/filters` | Open the filter surface |
...
| Playback | `d` | Open diagnostics |
| Playback | `o` | Choose source |
| Playback | `k` | Choose quality |
