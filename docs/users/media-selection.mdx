...
Press `k` during playback to change quality. Press `o` (or `/source`) for source. Press `s` to reload subtitles. Late subtitle lookup uses Wyzie only if you set a Wyzie API key in Settings › Language — Kunai ships none.
