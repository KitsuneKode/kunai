...
Browse mode uses `Tab` to cycle catalog mode (series / anime / YouTube). Narrow loaded results with `Ctrl+F`. Open filters with `/filters`. During playback, provider fallback is `⇧F` (`Shift+F`), not a bare `f`.
