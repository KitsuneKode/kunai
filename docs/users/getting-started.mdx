...
Anime episodes that will not play, or an empty search with AniDB selected, often mean **curl is missing** — Miruro (the default) and AniDB both sit behind Cloudflare. `kunai doctor` reports curl. See [Troubleshooting](/docs/users/troubleshooting).
