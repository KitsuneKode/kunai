...
and upcoming entries carry a separate status badge; post-live replays are
labelled separately when the backend exposes that state. Backends that omit a
shape or status signal are labelled conservatively. Live URLs
(`youtube.com/live/…`) resolve as videos. Kunai passes `--no-live-from-start`
to yt-dlp so a live handoff does not start from the beginning of the stream
unless you change that. Use `type:short` in YouTube search to narrow to Shorts;
the filter prefers yt-dlp/Piped results with an explicit Shorts signal.
...
| `poToken`            | Proof of Origin token (e.g. `web.gvs+...`) passed to yt-dlp for authenticated client extraction.                                             |
