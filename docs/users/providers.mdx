...
Search can return **videos, playlists, and channels**. Playlists and channels are catalog collections (`youtube-playlist:…` / channel ids) — pick a video inside them to play. Live URLs (`youtube.com/live/…`) resolve as videos; Kunai passes `--no-live-from-start` to yt-dlp so a live handoff does not start from the beginning of the stream unless you change that.
...
- Kunai does not host or proxy video. Enabling `videoFallback` in config does not route media through the relay in the current runtime.
