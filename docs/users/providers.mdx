...
<ProviderDocSection id="movy" title="Movy">

**Adapter role:** registered movies/series adapter backed by the movy.sx source aggregator. Each upstream lane (`denver`, `atlanta`, …) is a separate source, so `/source` switches lanes directly.
