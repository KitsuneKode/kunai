...
The notification inbox now has explicit action rows, history and browse rows can be queued without replacing playback, `/playlist` can import/export safe playlist JSON, and the experimental provider availability worker is wired as a cancellable no-network-by-default planning scaffold.
