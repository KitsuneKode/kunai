...
```sh
bun run --cwd .reference/experiments list
```

Run a provider probe:

```sh
bun run --cwd .reference/experiments vidking:0ram -- "breaking bad"
bun run --cwd .reference/experiments rivestream:headless -- "the matrix"
bun run --cwd .reference/experiments anikai:headless -- "one piece" 1 1159
bun run --cwd .reference/experiments miruro:headless -- "one piece" 1 1159
```
