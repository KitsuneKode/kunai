...
```jsonc
{
  "episode": {
    ...
    "episodeInfo": {
      "notes": "I'm Used to It",
      "thumbnails": ["/data2/ep_tbs/B6AMhLy6EQHDgYgBF/1_dub.jpg"],
      "vidInforssub": {
        "vidResolution": 1080,
...
`lastEpisodeInfo.notes` and `lastEpisodeDate` for the _last_ episode
...
`.archive/superpowers/plans/2026-05-26-kunai-fast-first-provider-runtime.md`)
...
1. **The persisted-GET timeout was tightened to 12s.** That is correct, but
   the 12s budget _also_ covers the time-to-first-byte of the API endpoint.
...
```
.reference/experiments/scratchpads/provider-allmanga/bench-resolution.ts
```
...
```sh
cd .reference/experiments && bun run scratchpads/provider-allmanga/bench-resolution.ts
```
...
- `.reference/experiments/scratchpads/provider-allmanga/bench-resolution.ts` — new benchmark
- `.reference/experiments/scratchpads/provider-allmanga/probe-page-images.ts` — Playwright probe (failed to bypass CF cleanly; superseded by direct API probe)
- `.reference/experiments/scratchpads/provider-allmanga/allmanga-network-capture.json` — empty (page never rendered past CF challenge)
- This report: `.reference/experiments/scratchpads/provider-allmanga/episode-metadata-and-latency.md`
