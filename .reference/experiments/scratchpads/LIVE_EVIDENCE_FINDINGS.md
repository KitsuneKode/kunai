...
Source: `bun .reference/experiments/scratchpads/live-evidence-harvest.ts` → `live-evidence-report.json`
...
```sh
cd .reference/experiments && bun run providers:browser-harvest
```
...
```sh
cd .reference/experiments && bun scratchpads/live-evidence-harvest.ts
```
