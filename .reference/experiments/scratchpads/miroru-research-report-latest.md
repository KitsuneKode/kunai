...
```ts
production_companies?.some((c) =>
  /Studio Ghibli|CoMix Wave|Madhouse|Bones|MAPPA|ufotable|Kyoto Animation|Wit Studio|Trigger|A-1 Pictures|Pierrot/.test(
    c.name,
  ),
);
```
...
function isLikelyAnime(item: {
  ...
  const ANIME_NETWORKS =
    /Nippon TV|Tokyo MX|MBS\b|Fuji TV|TV Tokyo|BS11|TOKYO MX|Animax|TVHokkaido|ytv|Yomiuri TV/;
  ...
  const ANIME_STUDIOS =
    /Studio Ghibli|CoMix Wave|Madhouse|Bones\b|MAPPA|ufotable|Kyoto Animation|Wit Studio|Trigger|A-1 Pictures|Pierrot|Sunrise|TMS Entertainment|OLM\b|P.A. Works|CloverWorks|Production I.G|Shaft\b|White Fox|Juniper|J.C.Staff|Silver Link/;
...
function isAnimeLikely(item): { isAnime: boolean; confidence: number; reason: string } {
  ...
  if (item.keywords?.some((k) => k.id === 210024))
    return { isAnime: true, confidence: 0.95, reason: "anime keyword" };
  if (
    item.networks?.some((n) =>
      /Nippon TV|Tokyo MX|MBS|Fuji TV|TV Tokyo|BS11|TOKYO MX|Animax/.test(n.name),
    )
  )
    return { isAnime: true, confidence: 0.95, reason: "anime network" };
  if (
    item.production_companies?.some((c) =>
      /Studio Ghibli|CoMix Wave|Madhouse|Bones|MAPPA|ufotable|Kyoto Animation|Wit Studio|Trigger|A-1 Pictures|Pierrot/.test(
        c.name,
      ),
    )
