...
- **Command Name:** Change the global bin command from `kitsunesnipe` to `kunai`.
  _(Example usage: `kunai watch "one piece"`, `kunai search "matrix"`)_
...
**Yes.** Refactoring into a monorepo is much easier _before_ the codebase becomes massive. Since we just finalized the core scraping logic, this is the perfect time to extract it into a package and set up Turborepo.
