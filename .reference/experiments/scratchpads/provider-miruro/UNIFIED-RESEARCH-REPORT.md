...
| Risk                                                 | Mitigation                                                                                                                                                                                         |
| ---------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Miruro rotates `PIPE_KEY` silently                   | Health check on every cycle, scrape `VITE_PIPE_OBF_KEY` from `_next/static/chunks/*.js` as fallback (probe exists in `.reference/experiments/scratchpads/provider-miruro/probe-key-extraction.ts`) |
...
Plus the original `.reference/experiments/scratchpads/provider-miruro/` scratchpads have earlier probe data from 2026-05-25 — useful for diffing PIPE_KEY rotation, server set changes, etc.
