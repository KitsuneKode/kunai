...
- **Auto-next handoff:** after EOF, Kunai shows the short auto-next countdown and then uses a prefetched stream immediately when available. If prefetch is still in flight, playback waits only for a brief grace window before falling back to normal resolve, so a slow prefetch cannot create a long hidden pause between episodes.
