...
- **Auto-next handoff:** after EOF, Kunai shows the short auto-next countdown and then uses a prefetched prepared stream when available. If prefetch is still in flight, playback waits up to the episode handoff budget (about eight seconds, with visible “Finishing next episode…” feedback) before falling back to normal resolve. Manual **N** uses the same handoff path and can start an urgent prefetch when near-EOF work never ran. **P** / episode picker cancel in-flight prefetch.
