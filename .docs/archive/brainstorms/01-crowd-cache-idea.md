...
You noted: _"Try not to scrape when possible so that we don't actually have to worry about compute for the webapp, cli is bring ur compute."_
...
1. **The CLI / Desktop Users (The Harvesters):**
   When a user runs `kunai watch "one piece" ep 1159` on their laptop, _their_ machine does the heavy Playwright scraping and Cloudflare bypassing.
2. **The Silent Push:**
   Once their local CLI successfully extracts the raw `.m3u8` stream link (e.g., `https://vault-99.owocdn...uwu.m3u8`), the CLI silently makes a tiny, authenticated POST request to our central `api.kunai.app` server. It says: _"Hey, AniList ID 21, Episode 1159 maps to this raw HLS link for the next 4 hours."_
