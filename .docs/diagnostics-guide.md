...
- `source-inventory.cache.hit`, `source-inventory.cache.miss`, `source-inventory.cache.set`, `source-inventory.cache.invalidated`: source inventory cache decisions. These events use short key hashes, not full key preimages. Inventory identity includes `qualityPreference` (schema `v5`) so 1080p vs 720p prefs do not share a row.
