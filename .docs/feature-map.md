...
| ------------------------------------------------------------------- | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| Flag parsing (35 flags)                                             | `apps/cli/src/cli-args.ts`, re-exported as `main.ts` `parseArgs()`     | `docs/users/cli-reference.mdx` (generated from `--help`)                                            |
...
| Playback target routing                    | `apps/cli/src/services/playback/playback-router.ts`, with local mpv adapted by `LocalPlaybackBackend.ts`              | [architecture.md](./architecture.md)                                             |
| Google Cast, LAN gateway, and experimental split audio | `apps/cli/src/services/playback/cast/*`, `apps/cli/src/services/playback/split-audio-playback-backend.ts`, `apps/docs/app/cast-receiver/route.ts` | [casting.md](./casting.md) |
...
| Media vocabulary and identity | `apps/cli/src/domain/media/*` — content kind (identity), `title.type` (structure), episode cursor, media-item adapters              | Locked nouns live in [adr/0001-personal-media-vocabulary.md](./adr/0001-personal-media-vocabulary.md). Anime films are kind `anime` and type `movie`. |
