import {
  ...
  const cArray = [
    ...
    "cE5SJG", "x1vj1", "HegbLe", "zJ3nmt4OA", "gt7rxW57dq", "clIE9b", "jyJ9g", "B5jXjMCSx",
    "cOzZBZTV", "FTXGy", "Dfh1q1", "ny9jqZ2POI", "X2NnMn", "MBtoyD", "qz4Ilys7wB", "68lbOMye",
    "3YUJnmxp", "1fv5Imona", "PlfvvXD7mA", "ZarKfHCaPR", "owORnX", "dQP1YU", "dVdkx", "qgiK0E",
    "cx9wQ", "5F9bGa", "7UjkKrp", "Yvhrj", "wYXez5Dg3", "pG4GMU", "MwMAu", "rFRD5wlM"
  ];
    ...
    async resolve(input, context) {
      ...
      try {
        ...
        const provRes = await fetch(
            ...
            { headers: { "User-Agent": USER_AGENT, Referer: RIVESTREAM_REFERER }, signal: context.signal }
        );
        ...
        for (const provider of providers) {
            ...
            try {
                const sourceRes = await fetch(url, { headers: { "User-Agent": USER_AGENT, Referer: RIVESTREAM_REFERER }, signal: context.signal });
                const sourceData = await sourceRes.json() as any;
                const rawSources = sourceData?.data?.sources || sourceData?.data || [];
        ...
        try {
            ...
            const subRes = await fetch(subUrl, { headers: { "User-Agent": USER_AGENT, Referer: RIVESTREAM_REFERER }, signal: context.signal });
