...
- **Implementation state (2026-08-16):** Deterministic implementation and
  regression work is complete. Data migration 030 adds a tracker-neutral
  reconciliation fact that commits in the same transaction as local
  list/history mutations; startup replay applies current identity and opt-in
  gates before creating an outbox row. Tracker sync remains experimental until
  the disposable-account CLI → SQLite outbox → restart recovery → remote
