...
| Attention, queue, notifications | **Foundation landed**: shared media action policy, rollout flags, recoverable queue sessions, notification/follow storage, playlist import/export/projection, inbox command. Open: richer action UI, visible continue/new-episode shelves, experimental provider availability scheduler. | [attention-queue-notifications-playlists.md](./attention-queue-notifications-playlists.md)           |
...
- **Attention/queue implementation** — foundation is now in code. Next work should wire shared media actions into history/recommendation/search rows, then add richer inbox/queue UI before enabling provider availability sync.
