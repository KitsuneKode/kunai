...
| Provider contract V2            | **In progress.** Additive shared types, schemas, app-domain threading, and history external-ID persistence are landed. Open: make third-party metadata fetches fallback-only, then migrate providers to populate the new fields and bump/source-inventory cache compatibility when byte-affecting metadata changes.                                                                                      | [provider-contract-v2.md](./provider-contract-v2.md)                                                 |
