...
| Attention, queue, notifications | **Foundation landed and hardened**: shared media action policy, rollout flags, recoverable queue sessions with explicit restore API, notification/follow storage, playlist import/export/projection/service, inbox command, deterministic refresh scheduler policy. Open: richer action UI, visible continue/new-episode shelves, provider availability worker integration behind the experimental flag. | [attention-queue-notifications-playlists.md](./attention-queue-notifications-playlists.md)           |
...
- **Attention/queue implementation** — foundation is now in code. Next work should wire shared media actions into history/recommendation/search row interactions, then add richer inbox/queue UI before enabling provider availability sync.
