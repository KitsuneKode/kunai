...
Today, Kunai’s richest playback path (**persistent session**, telemetry, segment skip chip, `N`/`P`/`i` routing from mpv back into the shell) assumes **Unix domain sockets** for mpv’s `--input-ipc-server`. On **Windows**, `ipcPath` is intentionally `null`, so:

- No IPC → weaker telemetry and no Bun-driven skip overlay contract.
- `resolveKunaiMpvBridgeScriptPath` skips the default bridge (unless the user sets an explicit `mpvKunaiScriptPath`).

That is a **correctness and UX gap** for a large share of users (Windows installs, some enterprise environments, beginners using Scoop/Chocolatey). Closing it should feel like **one product**: same diagnostics, same resume semantics, same skip chip behavior, same failure modes—only the transport differs under the hood.
...
| ------- | ----- | ----- | ------------------ | ----------------- |
| One-shot `launchMpv` | IPC when socket path set | Same | **No IPC** today (`ipcPath === null`) | Same as Linux if Bun + mpv run in WSL |
| Persistent `PersistentMpvSession` | Full IPC + bridge | Full IPC + bridge | **No IPC**, no default bridge | Full IPC + bridge if both processes in WSL |
| Bridge mirror path (`mpvBridgePath`) | Written under Kunai `configDir` | Same | Path exists; default bridge not loaded without IPC | Same as Linux in-guest |
