...
- Provider availability sync remains experimental/off by default. The implemented scheduler only plans eligible IDs; it does not perform provider calls.
...
- Add richer actionable inbox rows: restore queue, queue/download item, dismiss, and open details through `MediaActionRouter`.
- Wire `MediaActionRouter` into history, recommendations, search rows, and post-playback recommendation rows instead of only exposing the lower-level router.
...
- Provider availability worker integration should not be enabled until it has cancellation, budget telemetry, diagnostics, and no-N+1 tests.
