...
| 2   | Stats data source      | **`history_progress` only** for v1 — `duration_seconds` + `updated_at` grouping                                                                                                                                  | `playback_events` table exists but nothing writes to it. Building on the live table avoids a prerequisite instrumentation sprint.         |
...
| 7   | Calendar scope in v1   | Watchlist badge + add-to-playlist action fall out naturally once `ListService` exists. Week navigation (`[` / `]`) and released/upcoming filter are a **follow-up calendar pass**.                               | Calendar infrastructure (day headers, group labels, sort) is already solid. Integration is trivial; navigation needs its own design pass. |
...
| 12  | Stats progress bars    | **Episode count + total time watched, no series total denominator**. Bars are proportional to the most-watched show in the window.                                                                               | Total episode count requires TMDB/AniList calls per show — N+1 network risk. Honest local data is more reliable.                          |
