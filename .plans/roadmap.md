...
- Attention, queue, notifications, playlist sharing, and crash-recoverable queue execution now have a foundation in code: shared media actions, shared queue planning, background work lane scheduling, local inbox storage, queue recovery checkpoints/restore, playlist import/export helpers/service, deterministic refresh planning, and feature docs. Next is richer surface polish and provider availability sync behind the experimental flag: [.plans/attention-queue-notifications-playlists.md](.plans/attention-queue-notifications-playlists.md)
...
| Attention, queue, notifications   | In Progress   | [.plans/attention-queue-notifications-playlists.md](.plans/attention-queue-notifications-playlists.md)                      |
...
| YouTube provider                  | Stable        | [.plans/yt-provider.md](.plans/yt-provider.md) — v1 lane + Phase 4 polish (settings, diagnostics, SponsorBlock, live smoke) |
