...
> **For agentic workers:** REQUIRED SUB-SKILL: Use `superpowers:subagent-driven-development` (recommended) or `superpowers:executing-plans` when implementing this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Land the `ad11` reliability PR stack safely, fix regressions and merge conflicts those PRs introduce, then close the remaining reliability/UX gaps the sweep left open.

**Architecture:** Treat this as three layers: (1) integrate the six unique `ad11` branches onto `main` with conflict resolution, (2) patch P0/P1 defects introduced or exposed by that stack (YouTube diagnostics probe loss, shell-frame input merge, preserve/fallback polish, overlay effect pause, tests), (3) close planned follow-ups that none of the PRs addressed (footer/GO hint parity, cache-key migration note, diagnostics timeline/export retention, bounded error-shell hardening). Do not start subtitle preference learning, HLS benchmarking, or a full player-actor rewrite in this plan.

**Tech Stack:** Bun, TypeScript, Ink, existing diagnostics/playback/shell seams, `bun run test` / `typecheck` / `lint` / `fmt`.

**Source review:** Cloud-agent review of branches `cursor/*-ad11` vs `main` at `f6a5e1d7` (2026-07-10).
...
- [ ] **Step 3: Mark duplicate branch closed in PR description / comment**
...
- [ ] **Step 4: Commit nothing else; push integration branch**
...
- [ ] **Step 2: Merge keybind branch and expect `shell-frame.tsx` conflict**
...
- [ ] **Step 3: Resolve `shell-frame.tsx` to include BOTH behaviors**
...
- [ ] **Step 5: Commit conflict resolution if merge left uncommitted resolution**
...
- [ ] **Step 2: Merge playback cache/fallback**
...
- [ ] **Step 3: Merge provider-ui honesty; resolve `SourceInventoryService`**
...
- [ ] **Step 4: Full verification after stack merge**
...
- [ ] **Step 1: Write failing test for probe threading**
...
- [ ] **Step 2: Run test to verify current gap (or that fixture needs wiring)**

```bash
bun run test -- apps/cli/test/unit/app-shell/diagnostics-youtube-probe.test.ts
```

- [ ] **Step 3: Extend diagnostics overlay payload**
...
- [ ] **Step 4: Restore probe in `handleDiagnostics`**
...
- [ ] **Step 5: Pass probe into panel builder in `root-overlay-shell.tsx`**
...
- [ ] **Step 1: Introduce a small live-reason holder type**
...
- [ ] **Step 2: Replace defineProperty sites**
...
- [ ] **Step 3: Update all `input.cancellationReason` reads in resolve commit/policy paths to use `readCancellationReason(input)`**
...
- [ ] **Step 4: Run playback resolve tests**
...
- [ ] **Step 1: Extend pick result metadata (keep return type backward compatible)**
...
- [ ] **Step 2: Record diagnostics when `degradedAllDown`**
...
- [ ] **Step 3: Add unit test asserting `degradedAllDown: true` when every alternate is down**
...
- [ ] **Step 1: Gate the interval on `useRootContentInputSuspended()`**
...
- [ ] **Step 2: Add a small hook-level or component-level test** proving the interval is not scheduled when suspended (fake timers).
...
- [ ] **Step 2: Implement only if the overlay merge did not already cover this; otherwise strengthen assertions.**

- [ ] **Step 3: Strengthen `shell-screen-clear` test**
...
- [ ] **Step 1: If `schemaVersion` is part of the preimage, bump it** (e.g. `"source-inventory-v2"` → `"source-inventory-v3-quality"`) so old rows miss cleanly by design rather than colliding on partial keys.

- [ ] **Step 2: Add a one-line comment above `qualityPreference` in the preimage builder:**
...
- [ ] **Step 3: Add a unit test that two inputs differing only by `qualityPreference` produce different cache keys** (playback branch may already have this — verify and keep).
...
- [ ] **Step 1: Confirm resolve path still auto-selects materializable deferred streams**
...
- [ ] **Step 2: Soften hardsub mislabel risk**
...
- [ ] **Step 3: Tie stale threshold to inventory TTL when available**
...
- [ ] **Step 4: Run honesty tests and commit**
...
- [ ] **Step 2: Write failing keybinding/footer tests**
...
- [ ] **Step 3: Adjust `footerPriority` / `helpOnly` so help overlay is complete; keep footer capped via `selectFooterActions`**

- [ ] **Step 4: Update `.docs/keybindings.md` and mark P2 resolved in the playback reliability plan**
...
- [ ] **Step 1: Write failing test** that given recent provider timeline events, panel lines include attempt id / failure class / provider id without raw URLs.

- [ ] **Step 2: Implement `buildProviderAttemptTimelineLines(events): string[]`** and append from `buildDiagnosticsPanelLines`.

- [ ] **Step 3: Update diagnostics guide with one paragraph on the new section.**
...
- [ ] **Step 2: Call after writing a new export/trace file.**
...
- [ ] **Step 1: Audit current ErrorShell actions (Retry / Esc)** against `playback-problem.ts` scenarios; fill any missing primary action.

- [ ] **Step 2: Add/adjust unit tests for `toErrorScenario` coverage of provider-empty / timeout / user-cancel.**

- [ ] **Step 3: Document in v2 plan that full AppErrorBoundary remains deferred; this task closed the actionable gap.**
...
- [ ] **Step 1: Add a truth-index row** for this plan with status Implemented / In progress.

- [ ] **Step 2: Mark completed follow-ups in the source plans; leave explicit Non-Goals as deferred.**

- [ ] **Step 3: Changeset (patch)** summarizing user-visible fixes: diagnostics YouTube probe restore, overlay preserve, honest inventory age, safer cache preserve, footer/help parity.
