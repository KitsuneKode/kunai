...
console.log(
  JSON.stringify({
    ...base,
    // After `...base`, whose `ok` means only "a stream resolved". The matrix
    // reads this field and labels the row `evidence: "playback"`, so leaving
    // the resolve-time verdict here promotes a stream mpv could not open.
    ok,
    mpv,
