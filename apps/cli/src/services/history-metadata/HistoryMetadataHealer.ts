...
export type ResolvedHistoryMetadata = {
  /** Real catalog name, for a row still stored under a placeholder title. */
...
export class HistoryMetadataHealer {
  ...
  async heal(
    ...
    for (const target of targets) {
      ...
      const title = target.needsTitle ? resolved?.title : undefined;
      const episodeCount = resolved?.episodeCount;
      if (!title && !posterUrl && !hasExternalIds(externalIds) && !episodeCount) continue;

      if (title || posterUrl || hasExternalIds(externalIds)) {
