...
/** recharts allows a function `dataKey`, which is not a usable React key. */
const rowKey = (candidate: unknown, fallback: number): string | number =>
  typeof candidate === "string" || typeof candidate === "number" ? candidate : fallback;
      ...
      <div className="grid gap-1.5">
        {payload
          ...
          .map((item, index) => {
            ...
            return (
              <div
                key={rowKey(item.dataKey ?? item.name, index)}
                className={cn(
    ...
    >
      {payload
        ...
        .map((item, index) => {
          ...
          return (
            <div
              key={rowKey(item.dataKey ?? item.value, index)}
              className={cn(
