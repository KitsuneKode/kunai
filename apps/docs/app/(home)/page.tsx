...
const TerminalSimulator = memo(function TerminalSimulator({
  ...
  const terminalBodyRef = useRef<HTMLButtonElement>(null);
    ...
    <div className="grid min-h-[calc(100dvh-8rem)] grid-cols-[minmax(0,1.02fr)_minmax(360px,0.78fr)] items-center gap-12 pb-18 max-lg:min-h-0 max-lg:grid-cols-1 max-lg:pt-10">
      <div className="kunai-reveal">
        ...
        <div className="mt-8 flex flex-wrap gap-4">
          <a className="kunai-button kunai-button-primary" href={primaryCtaHref}>
            {primaryCtaLabel}
          </a>
          <a className="kunai-button" href={secondaryCtaHref}>
            {secondaryCtaLabel}
          </a>
          <button
            type="button"
      ...
      >
        ...
        <button
          ...
          aria-label="Terminal simulator display screen"
        >
...
export default function HomePage() {
  ...
  const flags = (metadata.cliOptions || []) as readonly CliOption[];
  ...
  const toggleFlag = (flagLong: string) => {
  ...
  const buildCommandLine = () => {
    let base = "kunai";
    const selectedObj = flags.filter((f) => selectedFlags.includes(f.long));
    const shortFlags = selectedObj.filter((f) => f.short).map((f) => f.short.replace("-", ""));
    const longFlags = selectedObj.filter((f) => !f.short).map((f) => f.long);

    if (shortFlags.length > 0) {
      base += ` -${shortFlags.join("")}`;
    }

    const hasSearch = selectedFlags.includes("-S") || selectedFlags.includes("--search");
    if (hasSearch) {
      base += ` -S "${searchWord}"`;
    }
    ...
    <main className="kunai-home mx-auto w-[min(1400px,calc(100vw-32px))] overflow-x-hidden py-8 max-md:w-[min(760px,calc(100vw-20px))]">
      ...
      <section className="kunai-final">
        ...
        <div className="flex flex-wrap gap-3">
          <a className="kunai-button" href={homeHero.primaryCta.href}>
