...
async function main(): Promise<number> {
  const localUrl = external ? "" : `postgres://kunai:kunai@127.0.0.1:${LOCAL_PORT}/kunai_analytics`;
  ...
  if (!external) {
    // Tear down first: a container left behind by an interrupted run, or by
    // `--keep`, still holds the previous run's rows. Starting from nothing is
    // the only state this script can reason about.
