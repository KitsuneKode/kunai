...
/**
 * Build the published series from rollups already ordered oldest to newest.
 *
...
export function buildPublicSeries(rollups: readonly DailyRollup[]): PublicAnalyticsSeries | null {
  ...
  const versionKeep = windowPublishableBuckets(rollups.map((r) => r.byVersion));
  const osKeep = windowPublishableBuckets(rollups.map((r) => r.byOs));
  const archKeep = windowPublishableBuckets(rollups.map((r) => r.byArch));

  const points = rollups.map((rollup) => ({
    day: rollup.day,
